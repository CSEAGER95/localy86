 ...
 */
ConditionCodes::ConditionCodes() : codes(0) {
...
bool ConditionCodes::getConditionCode(int32_t ccNum, bool &error)
{
   ...
   error = false;
   // Look at getBits in Tools.C.
   // Extract the single bit; if the bit is 1, the function returns 1.
   return (Tools::getBits(codes, ccNum, ccNum) == 1);
}
